...
const alarmSound = new Audio('../source-files/audio/alarm.mp3');

let pSave = 0;

function notifyPage(message) {
    if (!("Notification" in window)) {
        alert("Change brower if you want notifications");
...
function changeSession(timeActual, workingStateActual) {
    if (timeActual <= 0) {
        if (workingStateActual.session == true) {
            ...
            timeActual = modeValue.breakValue; // for timing because he deletes one second during execution
            timingState.play = false;
            timingState.pause = true;
            displayTimer(workingState, timeActual)
            alarmSound.play();
            notifyPage("Let's break !");
        } else {
            workingState.session = true;
            workingState.break = false;
            timeActual = modeValue.sessionValue; // for timing because he deletes one second during execution
            timingState.play = false;
            timingState.pause = true;
            displayTimer(workingState, timeActual)
            alarmSound.play();
            notifyPage("Let's work !");
        }
    }
    return (timeActual);
}
